...
//HANDLE this: user should not be able to set fft length from this class, however the method in the base class is public. Setting base class as private causes error in overlapadd.h ...
template <class T>
class FIRfilter : public FFT<T> {
public:
    FIRfilter(int length, string type, T samplerate, T fcutoff_low, T fcutoff_high);
	virtual ~FIRfilter();

    static constexpr T pi() { return T(3.14159265359); }

    void convolution(QVector<T> &sig, int sig_length, QVector<T> &filtered);
    void set_filter_len(int length);
    int get_filter_len();
    void display_resp(int abs_scale);

    void calculate_coefficients(string type, T samplerate, T fcutoff_low, T fcutoff_high );
...
protected:
    ...
    QVector<T> coefficients;
    QVector<T> save_chunk;

    QVector<T> dispcoef;  //used for displayinf coefficients
