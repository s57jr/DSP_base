...
template <class T>
class OverlapAdd : private FIRfilter<T>
{
public:
    OverlapAdd(int length, int signal_length, string filter_type, string window_type, T samplerate, T fcutoff_low, T fcutoff_high);
    virtual ~OverlapAdd();

    int get_sig_length();
    void set_sig_length(int length);
    ...
    void filter(QVector<T> &sig, QVector<T> &filtered);

private:

    int sig_length;

    QVector<T> ovadd_coef_resp_r;  //coefficients padded with 0s and fft-ed to be used in ovrlap-add
    QVector<T> ovadd_coef_resp_i;

    QVector<T> padding_zeros;

    QVector<T> save_add_r;
    QVector<T> save_add_i;

    QVector<T> ovaddsig_r;
    QVector<T> ovaddsig_i;
    QVector<T> ovaddsig1_r;
    QVector<T> ovaddsig1_i;
