...
template <class T>
class OverlapSave : public FIRfilter<T>
{
public:
    OverlapSave(int filter_length, int signal_length, string filter_type, string window_type, T samplerate, T fcutoff_low, T fcutoff_high);
...
private:
    ...
    QVector<T> ovsave_coef_resp_r;  //coefficients padded with 0s and fft-ed to be used in ovrlap-add
    QVector<T> ovsave_coef_resp_i;

    QVector<T> save_add;

    QVector<T> ovsavesig_r;
    QVector<T> ovsavesig_i;
    QVector<T> ovsavesig1_r;
    QVector<T> ovsavesig1_i;
